...
# PERMANENT fix for the "let down" vs "letdown" class of bug (confirmed live:
# a space-separated phrasing slipped past this regex, agent.py's
# _APPROACH_PROC_RE, AND subsection_router.py's _AD222_RE simultaneously,
# reaching free synthesis over an unscoped AD 2.22 chunk covering BOTH
# Maiduguri runways — every number quoted was technically present in the
# source, so the number-verifier passed a splice of RWY 05 and RWY 23 data).
#
# Keyword regexes on the QUESTION are a closed, guessable vocabulary; a pilot
# can always phrase a request in a way none of them anticipated. This checks
# the RETRIEVED CONTENT instead: the AIP's own structural marker for an
# approach-procedure block ("Instrument approach procedures for RWY... based
# on...") doesn't change no matter how the question was worded. If any
# candidate chunk is AD 2.22 and contains that marker, free synthesis is
# refused unconditionally. This is a second, independent trigger for the
# SAME safety outcome as _PROC_RE above -- not a replacement for it, since a
# quick literal-text check remains useful defense-in-depth.
_AD222_APPROACH_HDR_RE = re.compile(
    r"Instrument\s+approach\s+procedures?\s+for\s+RWY\s*\d{2}[LRC]?\s+based on\s+"
    r"[^\n0-9.]{2,80}", re.I)


def _content_is_unscoped_approach_procedure(results: List[AIPResult]) -> bool:
    """True if any retrieved chunk is AD 2.22 content containing the AIP's own
    approach-procedure header structure. Used as a query-phrasing-independent
    safety trigger -- see _AD222_APPROACH_HDR_RE above for why."""
    for r in results or []:
        sec = (getattr(r, "aip_section", "") or "").strip().upper()
        if sec.startswith("AD 2.22") and _AD222_APPROACH_HDR_RE.search(r.content or ""):
            return True
    return False
...
def synthesize_decision(question: str, results: List[AIPResult]) -> Tuple[str, object]:
    ...
    if _PROC_RE.search(question or "") or _content_is_unscoped_approach_procedure(results):
